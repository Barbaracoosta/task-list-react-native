...
const TarefasPraFazer = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const resultFiltered = result.filter(task => task.step === 'Para fazer');
  ...
  const nextStep = async (item) => {
    console.log("nextStep called with item:", item);
    setLoading(true);
    try {
      const step = 'Em andamento';
      await patchStep(item.id, step);
      await fetchData(); 
      setAlertMessage('Tarefa em andamento'); 
      setShowAlert(true); 
    } catch (error) {
...
export default TarefasPraFazer;
